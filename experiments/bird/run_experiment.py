...
class BIRDExperimentRunner:
    ...
    def __init__(self, bird_schema_loader, bird_data_loader, model_name="meta-llama/Meta-Llama-3-8B-Instruct", device="auto"):
    ...
    def _convert_bird_schema(self, db_id):
        ...
        columns = {t: bird_schema[t]["columns"] for t in tables}
        foreign_keys = {t: bird_schema[t]["foreign_keys"] for t in tables}

        return {"tables": tables, "columns": columns, "foreign_keys": foreign_keys}

    def _format_prompt(self, question, schema_info, evidence=""):
...
SQL:"""
    ...
    def run_hybrid(self, max_examples=None, output_file=None):
        ...
        print(f"\nRunning Hybrid on {n_examples} examples...")
        ...
        for idx in tqdm(range(n_examples), desc="Hybrid"):
            ...
            try:
                schema = self._convert_bird_schema(db_id)
    ...
    def run_unconstrained(self, max_examples=None, output_file=None, max_new_tokens=1024):
        ...
        print(f"\nRunning unconstrained on {n_examples} examples...")
...
def main():
    parser = argparse.ArgumentParser(description="Run experiments on BIRD")
    parser.add_argument("--data", required=True, help="Path to dev.json")
    parser.add_argument("--database-dir", required=True, help="Path to dev_databases/")
    ...
    runner = BIRDExperimentRunner(schema_loader, data_loader, args.model, args.device)
